...
function InboxFrame_Update()
	...
	local packageIcon, stationeryIcon, sender, subject, money, CODAmount, daysLeft, hasItem, wasRead, isGM, x, y, z;
...
-- Open Mail functions
function OpenMailFrame_OnShow()
	UIPanelWindows["MailFrame"].width = MailFrame:GetWidth() + OpenMailFrame:GetWidth() - 20;
	UpdateUIPanelPositions(MailFrame);
end

function OpenMailFrame_OnHide()
	UIPanelWindows["MailFrame"].width = MailFrame:GetWidth();
